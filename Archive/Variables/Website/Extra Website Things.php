...
if ($website_function_settings["is_prototype"] == False and $website_settings["custom_layout"] == False and $website_information["language"] != $language_geral) {
...
if ($website_information["type"] == $story_website_type and $website_information["language"] != $language_geral) {
	$text_to_show = "\n"."<script>"."\n".
	"Chapter_Number = 1;"."\n".
...
if ($website_information["type"] == $story_website_type and $story_website_settings["chapter_opener"] == True and $website_information["language"] != $language_geral) {
	echo "\n";
	echo '<script>'."\n";
...
if (isset($website_settings["custom_css_style"]) and $website_settings["custom_css_style"] == True and $website_information["language"] != $language_geral) {
	echo Create_Element("style", "", $custom_css_style);
}

if ($website_information["english_title"] == $website_titles["My Little Pony: Friendship Is Magic"]) {
	Show($jquery, $add_br = True);
}

if (file_exists($website_information["php_folder"]."Website Content.php")) {
	require $website_information["php_folder"]."Website Content.php";
}
