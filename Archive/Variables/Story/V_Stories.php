...
$stories_json_file = $story_database_folder."Stories.json";
$stories_json_string = Read_String($stories_json_file);
$stories_json = json_decode($stories_json_string);
...
$story_names_list = Read_Lines($story_database_files["Names"]);

foreach ($story_names_list as $story_name) {
	$local_english_story_name = $story_name;
	$local_portuguese_story_name = $story_name;

	if (str_contains(", ", $story_name) == True) {
		$local_english_story_name = explode(", ", $local_english_story_name)[0];
		$local_portuguese_story_name = explode(", ", $local_portuguese_story_name)[1];
	}
...
$story_website_links = array();

$i = 0;
foreach ($story_names_list as $story_website_name) {
	$local_english_story_name = explode(", ", $story_names_list[$i])[0];

	$local_story_folder = $mega_stories_folder.$local_english_story_name."/";
	$information_folder = $local_story_folder."Information/";
	$information_json_file = $information_folder."Information.json";

	$link = $local_english_story_name;

	if (is_file($information_json_file) == True) {
		$json = json_decode(file_get_contents($information_json_file), true);

		if (array_key_exists("Website link name", $json) == True) {
			$link = $json["Website link name"];
		}
	}

	$story_website_links[$local_english_story_name] = $website["url"].$link."/";
...
foreach ($story_statuses_text as $local_story_status) {
	$local_english_story_name = $story_names_list[$i];

	if (strpos($local_english_story_name, ", ") == True) {
		$local_english_story_name = explode(", ", $local_english_story_name)[0];
	}

	$story_name = $story_names[$local_english_story_name];

	if (strpos($story_name, ", ") == True) {
		$story_name = explode(", ", $story_name)[$language_number];
	}

	$story_statuses[$local_english_story_name] = $local_story_status;

	if ($website_information["language_title"] == $story_name) {
...
foreach ($english_story_names as $local_story_name) {
	$local_portuguese_story_name = $portuguese_story_names[$local_story_name];

	$local_story_cover = $website_media_images_story_covers.$local_story_name."/";

	if ($local_story_name != $local_portuguese_story_name) {
		$local_story_cover .= $website_information["full_language"]."/";
	}

	$extension = "png";

	if ($local_story_name == "SpaceLiving") {
		$extension = "gif";
	}
...
$english_titles_text = "Titles";
$titles_portuguese_text = "Títulos";
$titles_text = Language_Item_Definer($english_titles_text, $titles_portuguese_text);
...
$chapter_in_language = $chapters_text." ".$in_text." ".$website_information["full_language"];
...
$story_status_texts = Language_Item_Definer($english, $portuguese);
...
$read_story_text = Language_Item_Definer("Read story", "Ler história");
...
$commented_in_text = Language_Item_Definer("Commented in", "Comentado em");
$commented_on_the_chapter_text = Language_Item_Definer("Commented on the chapter", "Comentado no capítulo");
...
$write_texts_array = Language_Item_Definer($english, $portuguese);
...
$website_language_text = Language_Item_Definer_Per_Language("en/", "pt/");
...
$link = $Text -> format($website["netlify_format"], "spaceliving");

$spaceliving_discord_join_link = Language_Item_Definer($link."join_the_discord/", $link."entre_no_discord/");
...
if ($website_information["type"] == $story_website_type) {
	...
	"<".$h2_element.">".$middle_text."</".$h2_element.">"."\n".
	"<br /><br />"."\n".
	$margin."\n";
...
$text_template = '<{} class="{}"><b style="white-space: break-spaces;">{}</b></{}>';
...
foreach ($english_story_names as $value) {
	...
	$text = format($text_template, array($h2_element, $text_color, $local_story_name, $h2_element));

	$computer_card = '<a class="w3-btn '.$background_hover_white_css_class.' shake_side_to_side_animation '.$background.'" href="'.$story_website_link.'" '.$roundedborderstyle.">".$text.'<img class="'.explode(" ", $background)[1].'" src="'.$story_cover.'" width="650" style="border-radius: 5%;white-space: initial!important;"><br /><br /></a><br />';

	$text = format($text_template, array($h4_element, $text_color, $local_story_name, $h4_element));

	$mobile_card = '<a class="w3-btn '.$background_hover_white_css_class.' shake_side_to_side_animation '.$background.'" href="'.$story_website_link.'" '.$roundedborderstyle.">".$text.'<img class="'.explode(" ", $background)[1].'" src="'.$story_cover.'" width="230" style="border-radius: 9%;white-space: initial!important;"><br /><br /></a><br />';
...
$array = array(
	...
	"Rise to Equestria, Ascensão á Equestria",
	"My Little Pony - Friendship Is Magic - The Visit of Izaque",
	"The Life of Izaque in Equestria, A Vida de Izaque em Equestria",
);
