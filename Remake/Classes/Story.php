...
class Story extends Class_ {
	...
	public static function Chapter_Cover() {
		global $website;
		global $full_language;
		global $chapter_tab;
		global $i;
	...
	public static function Get_Chapter_Comments($comments_folder) {
		global $website;
		global $chapter_tab;
		global $width;
		global $margin;
	...
	public static function Get_Chapter_Reads($reads_folder) {
		global $website;
		global $chapter_tab;
		global $width;
		global $margin;
	...
	public static function Comment_And_Read_Buttons() {
		global $website;
		global $chapter_tab;
	...
	public static function Top_And_Bottom_Buttons() {
		global $website;
		global $chapter_tab;
		...
		if ($i != 1) {
			...
			$chapter_tab["top_button"] .= "\n\n\t".Story::Chapter_Button($c, $chapter_titles[$i - 1], $options);
		...
		$chapter_tab["bottom_button"] .= self::Comment_And_Read_Buttons();

		$chapter_tab["top_button"] .= "\n";
		$chapter_tab["bottom_button"] .= "\n";
	}

	public static function Chapter_Tab() {
		global $website;
		global $story;
		global $full_language;
		global $language;
		...
		if (file_exists($comments_folder) == True) {
			...
			$comments = self::Get_Chapter_Comments($comments_folder);
		...
		if (file_exists($reads_folder) == True) {
			# Get chapter reads
			$reads = self::Get_Chapter_Reads($reads_folder);
	...
	public static function Chapter_Tabs($chapter_titles) {
		global $website;
		global $story;
		global $full_language;
		global $language;
		...
		foreach ($chapter_titles as $chapter_title) {
			# Generate
			$chapter_tab = self::Chapter_Tab($chapter_title, $i);
