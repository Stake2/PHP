...
$sitephpfolder2 = $php_tabs_variable.$ano.'/';
$anosnumb = 3;

#VYears PHP files
$yearmakerfilephp = $php_tabs_variable.ucwords($sitetextmaker).'/YearMaker.php';
$yearmakerfilephp2 = $php_tabs_variable.ucwords($sitetextmaker).'/YearMaker2.php';
$yearmakerfilephp2test = $php_tabs_variable.ucwords($sitetextmaker).'/YearMaker2.php';
$yearsbuttonsgenerator = $php_tabs_variable.'Years/'.'YearsButtons Generator.php';
...
$anoanteriorbtn = '<button class="w3-btn '.$btnstyle.'" onclick="window.open('."'".$url."years/".$anoanterior."/'".')"><'.$n.'>'.$anoanterior.': <i class="fas fa-globe-americas"></i></'.$n.'></button>';

#Mobile previous year button
$anoanteriorbtnm = '<button class="w3-btn '.$btnstyle.'" onclick="window.open('."'".$url."years/".$anoanterior."/'".')"><'.$m.'>'.$anoanterior.': <i class="fas fa-globe-americas"></i></'.$m.'></button>';
...
if ($lang == $langs[0] or $lang == $langs[1]) {
	$taskmadefile = $notepad_folder_effort_variable.'/Will to Think and Register Thought/Productive Network/Archives/To-Do Slim '.$ano." Archive ".strtoupper($langs[1]).".txt";
}

if ($lang == $langs[2]) {
	$taskmadefile = $notepad_folder_effort_variable.'/Will to Think and Register Thought/Productive Network/Archives/To-Do Slim '.$ano." Archive ".strtoupper($langs[2]).".txt";
}
