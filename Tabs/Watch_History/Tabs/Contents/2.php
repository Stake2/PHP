...
while ($i <= $to_watch_line_number - 1) {
	...
	if ($website_watch_history_show_to_watch_only_setting == True) {
		...
		if (strpos ($to_watch_status_file_text[$i], $to_watch_string) == True) {
			...
			echo '<'.$h4_element.' class="'.$watch_status_css_class.' '.$text_hover_white_css_class.' '.$computer_variable.' '.$zoom_animation_class.'">'.$i2." - (".$to_watch_media_type_text[$i].") - ".$to_watch_text[$i].' - <a href="vlc://file:///'.$medias_local_folder.$to_watch_folders_text[$i].'/'.str_replace('"', "", $to_watch_text[$i]).'.mp4"><i class="fas '.$watch_status_icon.'"></i></a>'."</".$h4_element.">"."\n";
			echo "\n";
	...
	if ($website_watch_history_show_to_watch_only_setting == False) {
		...
		echo '<'.$h4_element.' class="'.$watch_status_css_class.' '.$text_hover_white_css_class.' '.$computer_variable.' '.$zoom_animation_class.'">'.$i2." - (".$to_watch_media_type_text[$i].") - ".$to_watch_text[$i].' - <a href="vlc://file:///C:/Midias/'.$to_watch_folders_text[$i].'/'.str_replace('"', "", $to_watch_text[$i]).'.mp4"><i class="fas '.$watch_status_icon.'"></i></a>'."</".$h4_element.">"."\n";
		echo "\n";
