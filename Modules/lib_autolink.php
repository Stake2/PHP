<?php
		...
		while (($cursor < strlen($text)) && $loop) {
			...
			if ($ok) {
				if (preg_match("/^([a-z0-9\-\.\/\-_%~!?=,:;&+*#@\(\)\$]+)/i", $post, $matches)){
					...
					if (array_key_exists("link_class", $GLOBALS) == True) {
						$class = $GLOBALS["link_class"];
					}

					$buffer .= '<a href="'.$link_url_enc.$currentTagfill.'"'.' target="_blank" class="'.$class.'">'.$display_url_enc."</a>";
				}
	...
	function Linkfy($text, $limit = 30, $tagfill = "", $auto_title = true) {
		return autolink($text, $limit = 30, $tagfill = "", $auto_title = true);
	}
?>
