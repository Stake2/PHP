...
if ($story_website_settings["has_story_covers"] == True) {
	...
	if (isset($website_information["image_format"]) == False) {
		$website_information["image_format"] = "png";
	}

	$website_image = $story_book_cover_folder.$story_book_cover_filename.".".$website_information["image_format"];
	...
	if (isset($website_information["image_format"]) == False) {
		$website_information["image_format"] = "jpg";
	}
...
$website_information["image_link"] = $website_image;
...
if (isset($custom_website_descriptions) == True) {
	$website_information["meta_description"] = $website_descriptions[$language_number];
	$website_information["header_description"] = $website_descriptions[$language_number];

	if ($website_information["english_title"] == $website_titles["Diary"] and isset($diary_blocks_explaining_text) == True) {
		$website_information["header_description"] .= $diary_blocks_explaining_text;
	}
}

if (isset($custom_website_descriptions) == False) {
	$website_information["meta_description"] = format(Language_Item_Definer("Website about my story, {}, made by Stake2, Funkysnipa Cat, Izaque.", "Site sobre a minha história, {}, feito por Stake2, Funkysnipa Cat, Izaque."), $website_story_name);

	$website_information["header_description"] = format($synopsis_text.': <i class="fas fa-scroll"></i> "{}"<br />', $story_synopsis);
