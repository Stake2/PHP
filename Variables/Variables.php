...
if (isset($website_information["theme_color"]) == False) {
	$website_information["theme_color"] = "#916f3b";
}

$website_information["language_title"] = Language::Item(["en" => $website_titles[$website_title], "pt" => $website_portuguese_titles[$website_title]]);
$website_information["language_title_with_icon"] = Language_Item_Definer($website_titles[$website_title], $website_portuguese_titles[$website_title]);
$website_information["english_title"] = $website_titles[$website_title];
$website_information["portuguese_title"] = $website_portuguese_titles[$website_title];
$website_information["type"] = $website_types[$website_title];
$website_information["link"] = $website_links[$website_title];
$website_information["php_folder"] = $website_php_folders[$website_titles[$website_title]];
$website_information["website_folder"] = $website_folders[$website_titles[$website_title]];
$website_information["author"] = $website["author"];
$website_information["date_format"] = Language_Item_Definer("H:i m/d/Y", "H:i d/m/Y");
$website_information["date_format_no_time"] = Language_Item_Definer("m/d/Y", "d/m/Y");

date_default_timezone_set("America/Sao_Paulo");
$todays_date = date($website_information["date_format_no_time"]);
...
if ($website_information["type"] == $story_website_type) {
	$website_story_name = $story_names[$website_information["english_title"]];
	$english_story_name = $english_story_names[$website_information["english_title"]];
	$portuguese_story_name = $portuguese_story_names[$website_information["english_title"]];
}

# Websites array
$i = 0;
foreach ($website_titles as $value) {
	if ($website_information["english_title"] == $value) {
...
$website_link_folder = Remove_Non_File_Characters($website_information["english_title"]);

if (isset($website_information["website_folder_name"]) == True) {
	$website_link_folder = $website_information["website_folder_name"];
}

$website_information["link"] = $website["url"].$website_link_folder."/";
...
$website_information["second_text_color_span"] = '<span class="'.$second_text_color.'">{}</span>';
...
if ($website_function_settings["tabs"] == True and $website_information["language"] != $language_geral) {
	require $website_tabs_generator;
}

if ($website_information["type"] == $story_website_type and $website_settings["notifications"] == True and $website_information["language"] != $language_geral) {
...
if ($website_settings["notifications"] == True and $website_information["language"] != $language_geral) {
...
Create_Database_Table($website_information["english_title"], $columns);

# Gets results from SQL Database
$sql = new SQL();

$columns = $sql -> select("SELECT * FROM ".strtolower(str_replace(" ", "_", $website_information["english_title"]))." WHERE language = '".$website_information["language"]."';");

# Adds the website info to the DB if there is no column there
if (count($columns) == 0) {
	$columns = array("language_title", "language_title_with_icon", "english_title", "portuguese_title", "type", "link", "php_folder", "website_folder", "meta_description", "header_description", "image_link");
	$values = array();

	foreach ($columns as $column) {
		if ($column != "php_folder" and $column != "website_folder") {
			array_push($values, $website_information[$column]);
		}

		if ($column == "php_folder" or $column == "website_folder") {
			array_push($values, $website_information[$column].$website_information["language"]."/");
		}
	}

	Insert_Into_Database_Table($website_information["english_title"], $columns, $values);
}
