...
$tabs_folder = $website_information["php_folder"]."Tabs/";
...
if ($website_information["english_title"] == $website_titles["Things I Do"]) {
	$i = 0;
	while ($i <= $website_tab_number) {
		if ($i < 1) {
			$text_size = "h6";	
...
$local_website_title = Remove_Non_File_Characters($website_information["english_title"]);
$local_website_title = str_replace(" ", "_", $local_website_title);

if ($website_information["english_title"] == $website_titles["Watch History"]) {
	# Tab codes array generator
	$i = 0;
	while ($i <= $website_tab_number) {
		if ($i < 3) {
			$tab_codes[$i] = $local_website_title."_".$tab_names[$i];
		}
...
$i = 0;
while ($i <= $website_tab_number) {
	$website_tab_codes_computer[$i] = $tab_codes[$i]."_Computer";
...
if ($website_settings["tab_body_generator"] == False) {
	...
	if (file_exists($website_information["php_folder"]."CityBody".$i2.".php")) {
		while ($i <= $website_tab_number) {
			$i2 = $i + 1;

			$city_body_files[$i] = $website_information["php_folder"]."CityBody".$i2.".php";
...
if ($website_information["english_title"] == $website_titles["Watch History"] and $website_information["language"] != $language_geral) {
...
if ($website_information["english_title"] == $website_titles["Things I Do"] and $website_information["language"] != $language_geral) {
	# Include the buttons loader PHP file
	require $buttons_generator;
}
...
$i = 0;
while ($i <= $website_tab_number) {
	if (isset($city_body_files[$i]) and $website_information["language"] != $language_geral) {
...
if ($website_settings["tab_body_generator"] == True and $website_information["language"] != $language_geral) {
...
while ($first_number <= $website_tab_number + 1 and $website_information["language"] != $language_geral) {
	$tab_contents_file = $website_information["php_folder"]."Tabs/Contents/".$second_number.'.php';
...
while ($i <= $website_tab_number) {
	...
	if (isset($tab_bodies[$i]) and $website_information["language"] != $language_geral) {
		...
		if (isset($tab_html_titles) and $website_information["language"] != $language_geral) {
...
if ($website_information["english_title"] != $website_titles["Things I Do"] and $website_information["english_title"] != $website_titles["Watch History"] and $website_function_settings["website_buttons"] == True and $website_information["language"] != $language_geral) {
...
$i = 0;
while ($i <= $website_tab_number and $website_information["language"] != $language_geral) {
	$i2 = $i + 1;
